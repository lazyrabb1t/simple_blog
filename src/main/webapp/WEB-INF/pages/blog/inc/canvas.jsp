...
<script>
	(function(window) {
		...
		Dotline.prototype.color2rgb = function(colorStr) {
			...
			if (cstr && cReg.test(cstr)) {
				if (cstr.length == 4) {
					var cstrnew = '#';
					for (var i = 1; i < 4; i++) {
						cstrnew += cstr.slice(i, i + 1).concat(
								cstr.slice(i, i + 1));
					}
					cstr = cstrnew;
				}
		...
		Dotline.prototype.move = function(dot) {
			...
			dot.y += dot.ay;
			//点碰到边缘返回
			dot.ax *= (dot.x > (this.c.width - this.radius) || dot.x < this.radius) ? -1
					: 1;
			dot.ay *= (dot.y > (this.c.height - this.radius) || dot.y < this.radius) ? -1
					: 1;
			//绘制点
			this.ctx.beginPath();
		...
		Dotline.prototype.drawLine = function(dots) {
			...
			this.dots.forEach(function(dot) {
				...
				for (var j = 0; j < dots.length; j++) {
					...
					// 如果是鼠标，则让粒子向鼠标的位置移动
					if (nowDot.label
							&& Math.sqrt(dc) > Math.sqrt(_that.disMax) / 2) {
						dot.x -= dx * 0.02;
						dot.y -= dy * 0.02;
					}
					var ratio;
					...
					_that.ctx.strokeStyle = 'rgba(' + _that.color + ','
