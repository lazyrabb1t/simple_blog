...
<title>${keyword }的搜索结果&nbsp;-&nbsp;lazyRabbit的博客</title>
		...
		<div id="main">
			...
			<c:if test="${not empty pageInfo.list}">
				<ul class="actions pagination">
					<li>
						<a href="/search?keyword=${keyword}&cp=${pageInfo.pageNum-1}" class="<c:if test="${pageInfo.isFirstPage || empty pageInfo.list}">disabled </c:if> button big previous">上一页</a>
					</li>
					<li>
						<a href="/search?keyword=${keyword}&cp=${pageInfo.pageNum+1}" class="<c:if test="${pageInfo.isLastPage  || empty pageInfo.list}">disabled </c:if>button big next">下一页</a>
					</li>
				</ul>
			</c:if>
		</div>
